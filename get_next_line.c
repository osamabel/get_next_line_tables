...
/*   Updated: 2021/12/19 20:02:28 by obelkhad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	line = NULL;
	if (lost_chars && check_end_of_line(lost_chars) <= ft_strlen(lost_chars))
	{
		ft_strsplit(lost_chars, &line, &lost_chars, check_end_of_line(lost_chars));
		if (check_end_of_line(line) < ft_strlen(line))
			return (line);
	}
	return (read_and_store(fd, &line, &lost_chars));
}
